...
async function main() {
    ...
    if (network.name === "hh") {
        ...
        oracleWallet = new ethers.Wallet(process.env.ORACLE_HH_PRIVATE_KEY, ethers.provider);
    } else {
        oracleWallet = new ethers.Wallet(process.env.ORACLE_PRIVATE_KEY, ethers.provider);
    }
    ...
    const ModuleNonSanctioned = await ethers.getContractFactory(`${path}/compliance/modular/modules/NonSanctionModule.sol:NonSanctionModule`);
    ...
    const moduleNonSanctioned = await ModuleNonSanctioned.deploy();
    deploymentsReceipts["moduleNonSanctioned"] = await moduleNonSanctioned.deploymentTransaction().wait();
    ...
    // Compliance module inclussion
    const moduleInit = await moduleNonSanctioned.connect(signers[0]).init(oracleAddress);
    initReceipts["modularInit"] = await moduleInit.wait();

    const modularAddition = await modularCompliance.connect(signers[0]).addModule(
        moduleNonSanctioned.target
    );
    initReceipts["modularAddition"] = await modularAddition.wait();
    ...
    await token.connect(signers[0]).mint(signers[1].address, 100000);
    ...
    contract_addresses[network.name]["deployment"]["contracts"]["moduleNonSanctioned"] = {
        address: moduleNonSanctioned.target,
        deploymentGas: deploymentsReceipts["moduleNonSanctioned"].gasUsed.toString(),
        deploymentFee: deploymentsReceipts["moduleNonSanctioned"].fee.toString(),
    };
