...
contract ERC1400 is IERC20, IERC1400, Ownable, MinterRole {
  ...
  function balanceOf(address tokenHolder) external override view returns (uint256) {
    return _balances[tokenHolder]; // TODO: CHANGE IT TO DEFAULT PARTITION ONLY BALANCE, NON-COMPLIANT, BUT EASIER TO INTEGRATE WITH ERC20???? UNCLEAR STANDARD
  }
  /**
   ...
   * @param value Number of tokens to transfer.
   */
  // TODO: Make this operation exclusive for default partitions.
  // Remember this is NOT standard, but it is a common error, as it happened to me
  ...
  /**
   * @dev Transfer tokens from a specific default partition.
   * @param defaultPartition Default partition where tokens transfers occurs.
   * @param operator The address performing the transfer.
   * @param from Token holder.
   * @param to Token recipient.
   * @param value Number of tokens to transfer.
  ...
  returns (bytes32)
  {
    require(_balanceOfByPartition[from][defaultPartition] >= value, "52"); // 0x52	insufficient balance

    //_removeTokenFromPartition(from, defaultPartition, value);
    _transferWithData(from, to, value);
    //_addTokenToPartition(to, toPartition, value);

    emit TransferByPartition(defaultPartition, operator, from, to, value, data, operatorData);
  ...
  internal
  {
    ...
    for (uint i = 0; i < _defaultPartitions.length; i++) {
      ...
      if(_remainingValue <= _localBalance) {
        ...
        _transferByDefaultPartition(_defaultPartitions[i], operator, from, to, _remainingValue, data, "");
        _remainingValue = 0;
        break;

      } else if (_localBalance != 0) {
        //_transferByPartition(_defaultPartitions[i], operator, from, to, _localBalance, data, "");
        _transferByDefaultPartition(_defaultPartitions[i], operator, from, to, _localBalance, data, "");
  ...
  internal
  {
    ...
    emit TransferByPartition(toPartition, operator, address(0), to, value, data, "");
  }
  /************************************************************************************************/
  ...
  function _setControllers(address[] memory operators) internal {
    for (uint i = 0; i < operators.length; i++) {
      ...
      for (uint j = 0; j < _defaultPartitions.length; j++) {
        _isControllerByPartition[_defaultPartitions[j]][operators[i]] = true;
        _controllersByPartition[_defaultPartitions[j]].push(operators[i]);
      }
    }
  }
