...
async function main() {
    ...
    const issueTransaction = await token.connect(signers[0]).issueByPartition(defaultPartitions[0], signers[0].address, 10000, "0x00");
    const issueReceipt = await issueTransaction.wait();
    ...
    //await token.connect(signers[0]).issue(signers[1].address, 100000, "0x00");
    let result;
    result = await token.connect(signers[0]).issueByPartition(defaultPartitions[0], signers[0].address, 10000, "0x00");
    await result.wait();
    ...
    contract_addresses[network.name]["deployment"]["contracts"]["Token"] = {
        ...
        deploymentGas: receipt.gasUsed.toString(),
        deploymentLatency: latency,
        deplyomentFee: receipt.fee.toString(),
