...
async function main() {
    ...
    const amount = 1n;
    const parition = ethers.hexlify(ethers.zeroPadValue("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef", 32));

    const resultsFile = "results.csv";

    const testingInterations = 100;
    ...
    for (let i = 0; i < testingInterations; i++) {
        ...
        const tx = await token.connect(sender).transferByPartition(
            parition,
            receiver.address,
            amount,
            "0x00"
        ...
        csvData += `TransferByPartition, ${receipt.gasUsed.toString()}, ${receipt.fee.toString()}, ${latency}, ${network.name}\n`;
    }
