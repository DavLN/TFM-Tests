...
contract Identity is IIdentity {
    ...
    */
     ...
     */
    function execute(address _to, uint256 _value, bytes memory _data) external delegatedOnly override payable returns (uint256 executionId) {
     ...
     */
    function getKey(bytes32 _key) external override view returns(uint256[] memory purposes, uint256 keyType, bytes32 key) {
    ...
    */
    function getKeyPurposes(bytes32 _key) external override view returns(uint256[] memory _purposes){
    ...
    */
    function getKeysByPurpose(uint256 _purpose) external override view returns(bytes32[] memory keys) {
    ...
    */
    function getClaimIdsByTopic(uint256 _topic) external override view returns(bytes32[] memory claimIds) {
    ...
    */
    function addKey(bytes32 _key, uint256 _purpose, uint256 _type) public delegatedOnly onlyManager override returns (bool success){
     ...
     */
    function approve(uint256 _id, bool _approve) public delegatedOnly override returns (bool success) {
    ...
    */
    function removeKey(bytes32 _key, uint256 _purpose) public delegatedOnly onlyManager override returns (bool success) {
    ...
    */
    function addClaim(
        ...
        bytes memory _data,
        string memory _uri
    ) public delegatedOnly onlyClaimKey override returns (bytes32 claimRequestId) 
    {
        //if (_issuer != address(this)) { // Consider changing it to the creator
        if (_issuer != address(_owner)) { // Modified to use _owner, as the original implementation used a factory to mask this
    ...
    * specified _claimId (topic, scheme, signature, issuer, data, uri) .
    */
    function getClaim(bytes32 _claimId) public override view returns(
        uint256 topic,
        uint256 scheme,
        address issuer,
    ...
    */
    function keyHasPurpose(bytes32 _key, uint256 _purpose) public override view returns(bool result) {
    ...
    function isClaimValid(
        ...
        uint256 claimTopic,
        bytes memory sig,
        bytes memory data) public virtual view returns (bool claimValid)
    {
